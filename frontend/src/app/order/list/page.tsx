        ...
        <div className='border my-2 rounded-xl flex border-gray-300'>
            ...
            <div className='border-l border-l-gray-300 ml-3 px-2'>
                <div className='border rounded my-2 border-gray-300'>
                    <Link href={`/order/delivery/${orderItem.orderItem_id}`} className='text-center block m-1 p-1'>배송 조회</Link>
                </div>
                <div className='border rounded my-2 border-gray-300'>
...
function OrderList({data}:{data:OrderResponseDto}){
    return (
        <div className='p-3 rounded-xl my-3 shadow-[0_0_5px_rgba(0,0,0,0.3)]'>
            ...
            {data.order_items!.map((e)=><OrderItem orderItem={e} key={e.orderItem_id}></OrderItem>)}
...
export default function page() {
    ...
    const onSubmit = async (e:FormEvent<HTMLFormElement>) =>{
        ...
        try{
            // const 
            const response = await fetch(`http://localhost:8080/api/orders/search?memberEmail=${memberEmail}`)
        ...
        <div className='mt-10 text-black'>
            <div className='flex justify-center'>
                <form onSubmit={onSubmit}>
                    <input type='email' value={memberEmail} onChange={(e)=>setMemberEmail(e.target.value)} className='border p-2' placeholder='이메일 주소를 작성해 주세요.'></input>
                    <button className='border ml-3 rounded p-2 cursor-pointer' type='submit'>조회</button>
                </form>
            </div>
            <div className='flex justify-center mt-3'>
                ...
                <p>조회된 주문이 없습니다.</p> :
                <ul>
                    {orders.map((e)=><OrderList data={e} key={e.order_number}></OrderList>)}
                </ul>}
